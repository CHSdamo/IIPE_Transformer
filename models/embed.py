...
class LabelEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = self.label_embedding(x[:, :, 0].long())
        return x
...
class ValueEmbedding(nn.Module):
    ...
    def forward(self, x):
        x = self.value_conv(x[:, :, -1].float().unsqueeze(-1).permute(0, 2, 1)).transpose(1, 2)
        return x
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, mark):
        x_embd = self.value_embedding(x) + self.label_embedding(x) \
                + self.token_embedding(mark) + self.position_embedding(x)
