...
class Transformer(nn.Module):
    def __init__(self, args, dataset):
        ...
        self.decoder = Decoder(args, dataset)
        self.projection1 = nn.Linear(args.d_model, self.vocab_size, bias=False)
        self.projection2 = nn.Linear(args.d_model, 1)
        # self.projection3 = nn.Linear(args.d_model, 4)

    def forward(self, enc_inputs, dec_inputs, batch_x_mark, batch_y_mark):
        ...
        enc_outputs, enc_self_attns = self.encoder(enc_inputs, batch_x_mark)

        # dec_outputs: [batch_size, tgt_len, d_model],
        # dec_self_attns: [n_layers, batch_size, n_heads, tgt_len, tgt_len],
        # dec_enc_attn: [n_layers, batch_size, tgt_len, src_len]
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(dec_inputs, enc_outputs, batch_y_mark)

        # dec_outputs: [batch_size, tgt_len, d_model] -> dec_logits: [batch_size, tgt_len, tgt_vocab_size]
        dec_logits = self.projection1(dec_outputs)
        reg_output = self.projection2(dec_outputs)
        # err_type_logits = self.projection3(dec_outputs)
